...
def train_meta_epoch(args: CFG, epoch, trainloader, normalizing_flow, optimizer, POS_EMB: PositionalEncoding1D,
                     metric_recoder: MetricRecoder, boundary_class: Boundary_recoder = None):
    """
    :param args:
    :param epoch:
        ...
        for (i, loader) in enumerate(trainloader):
            ...
            if epoch == 0:
                ...
                if sub_epoch < args.normal_sub_epoch:
                    ...
                    boundaries = boundary_class.boundary
                ...
                if math.isnan(loss_item):
                    ...
                    total_loss += loss.item()
                    loss_count += 1
            logps_list.append(logps.detach().cpu())
            label_list.append(m_b)
...
def train(args: CFG):
    ...
    boundary_class = Boundary_recoder()
    for epoch in range(start_epoch, args.num_epochs):

        logps_list, label_list = train_meta_epoch(args, epoch, trainloader, normalizing_flow, optimizer, pos_embed,
                                                  metric_recoder=train_recoder, boundary_class=boundary_class)
        if epoch % args.update_boundary_frequency == 0:
            print("Update Boundary")
            logps_tensor = torch.hstack(logps_list)
            label_tensor = torch.hstack(label_list)
            boundaries = get_logp_boundary(logps_tensor, label_tensor, args.pos_beta,
                                           args.margin_abnormal_negative,
                                           args.margin_abnormal_positive,
                                           args.normalizer)
            boundary_class.update(boundaries)
