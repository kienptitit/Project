...
if __name__ == '__main__':
    ...
    f1 = torch.randn(3, 3)
    f2 = torch.randn(3, 3)
    g1 = dgl.graph(([0, 0, 1, 1, 2, 2], [1, 2, 0, 2, 0, 1]))
    g1.ndata['x'] = f1
    g2 = dgl.graph(([0, 0, 1, 1, 2, 2], [1, 2, 0, 2, 0, 1]))
    g2.ndata['x'] = f2
    graph = [g1, g2]

    model = GAT(3, 3, 3, 1, 1, 1, "relu", 0.1, 0.1, 0.1, True, "batchnorm")
    model.eval()
    print(model(g1, g1.ndata['x']))
    print(model(g2, g2.ndata['x']))
    # print(g1.ndata['x'])
    # print(g2.ndata['x'])
    ...
    data = Mydataset(graph)
    tmp_loader = DataLoader(data, batch_size=2, collate_fn=collate)
    feat = torch.randn(2, 3, 3)
    for x in tmp_loader:

        print(model(x, x.ndata['x']))
